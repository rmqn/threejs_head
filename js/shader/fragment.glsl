...
void main(){
  ...
  vec2 newFakeUV=vec2((angle+PI)/(1.*PI),phi/PI);
  vec2 fakeUV=vec2(dot(vec3(1),vNormal),dot(vec3(-1.,0.,1.),vNormal));
  
  fakeUV=fract(fakeUV+vec2(time/40.,time/200.));
  
  vec4 txt=texture2D(sky,newFakeUV+.2*cnoise(vec4(fakeUV*5.,time/100.,0.),vec4(5.)));
